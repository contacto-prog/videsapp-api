...
/* ==================== HEADERS & FETCH ==================== */
export function headers(base, accept = "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8") {
  return {
    "user-agent":
    ...
    "pragma": "no-cache",
    ...(base ? { referer: String(base).replace(/\/?$/, "/") } : {}),
  };
}

export async function getText(url, hdrs = {}, timeoutMs = 15000) {
  const ac = new AbortController();
  const t = setTimeout(() => ac.abort(), timeoutMs);
  try {
    const res = await fetch(url, { headers: hdrs, signal: ac.signal, redirect: "follow" });
    let text = "";
    try {
      text = await res.text();
    } catch {}
...
export async function extractJsonLdPrices(page) {
  ...
  for (const txt of jsons) {
    try {
      ...
      for (const d of arr) {
        const price =
          d?.offers?.price ?? d?.offers?.lowPrice ?? d?.price ?? d?.offers?.[0]?.price ?? null;
        if (price != null) {
  ...
  try {
    ...
    for (const t of texts) {
      const btn = await page.$x(
        `//button[contains(translate(.,"ACEPTO","acepto"), "${t.toLowerCase()}")]`
      );
      if (btn?.length) {
...
async function tryVtexState(page) {
  try {
    ...
    const items = [];
    const re =
      /"commertialOffer"\s*:\s*\{[^}]*"Price"\s*:\s*([0-9.]+)[^}]*\}[^}]*\}\s*,\s*"name"\s*:\s*"([^"]+)"/gi;
    let m;
...
export async function tryVtexSearch(page, product, mapItem = (p) => p) {
  try {
    ...
    const scs = ["", "&sc=1", "&sc=2"];
    const orders = ["", "&O=OrderByPriceASC"];
    const windows = [
      { from: 0, to: 19 },
      { from: 0, to: 40 },
      { from: 0, to: 60 },
...
export async function pickCards(page, sels) {
  ...
  const raw = await page
    .$$eval(
      cards,
      (nodes, nameSels, priceSels, linkSels) => {
        const pick = (el, sels) => {
          for (const s of sels) {
  ...
  try {
    ...
    for (const t of labels) {
      const btns = await page.$x(
        `//button[contains(translate(normalize-space(.),"ACEPTAROKCERRARSELECTCONFIRM","aceptarokcerrarselectconfirm"), "${t.toLowerCase()}")]`
      );
      if (btns?.length) {
        await btns[0].click({ delay: 30 }).catch(() => {});
        await page.waitForTimeout(200);
      }
    }
    for (const s of sels) {
      ...
      if (n) {
        await n.click({ delay: 30 }).catch(() => {});
        await page.waitForTimeout(200);
      }
    }

    const overlays = await page.$$(
      '[role="dialog"], [class*="modal"], [class*="dialog"], .ReactModal__Overlay'
    );
    if (overlays?.length) {
...
/* ============== Compat: nombres legacy usados por otros módulos ============== */
export const priceCLPnum = parsePriceCLP;
