...
// Fallbacks genéricos por fuente (URL y selectores candidatos)
const FALLBACK = {
  [cv]: {
    buildUrl: (q) => `https://www.cruzverde.cl/search?q=${encodeURIComponent(q)}`,
    selectors: ['[data-product-id]', '.product-tile', '.product-list__item', 'article'],
    nameSel: ['h3','h2','.product-title','a[title]'],
    priceSel:['.price','.product-price','[class*="price"]'],
  },
  [sb]: {
    buildUrl: (q) => `https://salcobrand.cl/search?q=${encodeURIComponent(q)}`,
    selectors: ['.product', '.product-grid__item', '[data-sku]', 'article'],
    nameSel: ['h3','h2','.product-title','.title','a[title]'],
    priceSel:['.price','.product-price','[class*="price"]'],
  },
  [ah]: {
    buildUrl: (q) => `https://www.farmaciasahumada.cl/search?q=${encodeURIComponent(q)}`,
    selectors: ['.product-grid-item', '.product-item', '[data-product-id]', 'article'],
    nameSel: ['h3','h2','.product-title','.title','a[title]'],
    priceSel:['.price','.product-price','[class*="price"]'],
  },
  [fe]: {
    buildUrl: (q) => `https://farmex.cl/search?q=${encodeURIComponent(q)}`,
    selectors: ['.product-card', '.product', 'article', '.grid-item'],
    nameSel: ['h3','h2','.product-title','.title','a[title]'],
    priceSel:['.price','.product-price','[class*="price"]'],
  },
  [ds]: {
    buildUrl: (q) => `https://www.drsimi.cl/catalogsearch/result/?q=${encodeURIComponent(q)}`,
...
function buildGenericExtractor(sourceId) {
  const cfg = FALLBACK[sourceId];
  if (!cfg) return null;

  const { selectors, nameSel, priceSel } = cfg;

  return async function genericExtract(page, q) {
    // Espera a que el DOM base esté disponible y haz un pequeño idle
    await page.waitForSelector('body', { timeout: 15000 }).catch(()=>{});
    await page.waitForNetworkIdle({ idleTime: 800, timeout: 15000 }).catch(()=>{});

    // Algunos sitios requieren cerrar/aceptar cookies
    try {
      const cookieBtn = await page.$x(
        "//button[contains(translate(., 'ACEPTAR', 'aceptar'),'acept') or contains(., 'Aceptar') or contains(., 'aceptar')]"
      );
    ...
    // Scrollea para forzar carga lazy
    try {
    ...
    // Extrae con múltiples selectores candidatos
    const items = await page.evaluate(({ selectors, nameSel, priceSel, sourceId }) => {
      ...
      for (const el of nodes) {
        const nameEl =
          el.querySelector(nameSel.join(',')) ||
          el.closest('article, .product, .product-item, .product-card, .grid-item')?.querySelector(nameSel.join(','));
        ...
        if (name && url && price) {
          take.push({ source: sourceId, name, price, url });
        }
        if (take.length >= 30) break; // límite razonable por fuente
      }
      return take;
...
async function runFallbackForSource(page, sourceId, q) {
  const cfg = FALLBACK[sourceId];
  if (!cfg) return [];
  ...
  // Intenta también un segundo hit de networkidle2 si es SPAs
  try {
    await page.waitForNetworkIdle({ idleTime: 1000, timeout: 10000 });
  } catch {}
  await dumpPage(page, sourceId, 'fallback-networkidle');

  const extractor = buildGenericExtractor(sourceId);
  if (!extractor) return [];
  const rows = await extractor(page, q);
...
export async function scrapePrices(product) {
  ...
  try {
    for (const s of SOURCES) {
      ...
      const runOnce = async (label, timeoutMs) => {
        // Cada scraper específico DEBE usar esta `page`
      ...
      // 1) Intento con el scraper específico
      try {
        rows = await runOnce(s.id, s.timeoutMs);
      } catch (e1) {
        // Reintento con timeout más corto
        try {
          const retryMs = Math.max(8000, Math.floor(s.timeoutMs * 0.6));
          rows = await runOnce(`${s.id}_retry`, retryMs);
        } catch (e2) {
          errors.push({ source: s.id, error: `scraper_error: ${String(e2?.message || e2)}` });
        }
      }

      // 2) Si no hay resultados, aplica fallback genérico
      if (!rows.length) {
        try {
          usedFallback = true;
          const fbRows = await runFallbackForSource(page, s.id, key);
          rows = fbRows;
          if (!rows.length) {
            errors.push({ source: s.id, error: 'fallback_empty' });
          }
        } catch (e3) {
      ...
      if (!rows.length) await dumpPage(page, s.id, usedFallback ? 'empty-after-fallback' : 'empty-after-scraper');
    }
  } finally {
  ...
  const result = {
    ...
    errors, // deja trazabilidad (scraper_error / fallback_empty / fallback_error)
    note: 'Agregación multi-fuente con fallback genérico; cada fuente tiene timeout y reintento. HTML puede cambiar.',
  };
