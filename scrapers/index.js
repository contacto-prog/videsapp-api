...
// Fallbacks genéricos por fuente
const FALLBACK = {
  [cv]: {
...
// Llama a un scraper en cualquiera de sus 2 firmas soportadas.
// 1) (page, q)  -> usa la page compartida
// 2) (q, {puppeteer}) -> el scraper maneja su propio browser
async function callScraperFlexible(s, page, q) {
  ...
  try {
    ...
    // seguimos al intento B
    log(s.id, 'firma (page,q) lanzó error:', e?.message || e);
  }

  // Intento B: (q, { puppeteer })
  try {
    ...
    log(s.id, 'firma (q,{puppeteer}) lanzó error:', e?.message || e);
    throw e; // para que el caller maneje retry y logging
  }

  // Si ninguna firma devolvió array, retornamos vacío
  return [];
}
...
async function runFallbackForSource(page, sourceId, q) {
  ...
  if (DEBUG) log(sourceId, 'fallback_items:', rows.length);
...
export async function scrapePrices(product) {
  ...
  try {
    for (const s of SOURCES) {
      ...
      // 2) Fallback genérico si no trajo nada
      if (!rows || !rows.length) {
        usedFallback = true;
        try {
  ...
  const result = {
    ...
    errors,
    note: 'Agregación multi-fuente con fallback y firma flexible de scrapers.',
  };
